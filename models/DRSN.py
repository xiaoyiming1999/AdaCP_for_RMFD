...
class RSNet(nn.Module):
    ...
    def __init__(self, block, num_block, output_dim):
        super().__init__()

        self.in_channels = 64

        self.conv1 = nn.Sequential(
            nn.Conv1d(1, 64, kernel_size=25, stride=2, padding=0, bias=False),
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True))
...
def rsnet18(out_channel):
    """ return a RSNet 18 object
    """
    return RSNet(BasicBlock, [2, 2, 2, 2], out_channel)


def rsnet34(out_channel):
    """ return a RSNet 34 object
    """
    return RSNet(BasicBlock, [3, 4, 6, 3], out_channel)


if __name__ == '__main__':
    model = rsnet18(out_channel=8)
    input = torch.randn(size=[2, 1, 1024])
    output = model(input)
